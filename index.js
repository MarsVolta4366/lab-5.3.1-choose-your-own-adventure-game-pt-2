...
function continuePath() {
    let answer = window.prompt("You come across a chamber that extends upward to a shining light above. There is a long, winding staircase and a much quicker, but rickety-looking, ladder that lead up towards the light. Which do you take?")
    if(answer === "back") {
        goLeft()
...
function goRight(){
    let answer = window.prompt(`You come across a snoring dragon. On the other side of him, you see a shiny chest of treasure. Another path would lead you away from the dragon all together. Which do you take?`)
    if(answer === 'back'){
        start()
    } else if(answer === "past the dragon") {
        pastTheDragon()
...
function pastTheDragon() {
    let answer = window.prompt("The dragon awakes and sits upright. You only have a moment to respond, to stay or to run:")
    if(answer === "back") {
        goRight()
    } else if(answer === "stay") {
...
function awayFromTheDragon() {
    ...
    if(answer === "back") {
        ...
        alert("You draw the flower, capturing only a fraction of its beauty with your quill. You bring the drawing home, somewhat disappointed, but over time discover joy in sharing the drawing with your friends and family, recounting the story of your days as a sorcerer with the aide of the sketch.")
